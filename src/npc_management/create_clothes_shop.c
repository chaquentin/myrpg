...
icon_clothes_t **create_all_icon_clothes(game_t *game)
{
    ...
    all_clothes[0] = create_shirt(game, TankShop, 0, 1);
    all_clothes[0]->owned = 1;
    all_clothes[1] = create_shirt(game, BlackShop, 15, 5);
    all_clothes[2] = create_shirt(game, FlowerShop, 25, 15);
    all_clothes[3] = create_shirt(game, FireShop, 50, 25);
    all_clothes[4] = create_shirt(game, PinkShop, 85, -40);
    all_clothes[5] = create_hat(game, BlackCapShop, 15, 3);
    all_clothes[6] = create_hat(game, BlackDouragShop, 8, 15);
    all_clothes[7] = create_hat(game, FireCapShop, 25, 20);
    all_clothes[8] = create_pants(game, BluePantsShop, 0, 1);
    all_clothes[8]->owned = 1;
    all_clothes[9] = create_pants(game, BlackPantsShop, 15, 2);
    all_clothes[10] = NULL;
    return all_clothes;
}
